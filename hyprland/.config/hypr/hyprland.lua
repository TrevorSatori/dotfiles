...
-- -- Add this state variable somewhere above your keybindings section
local grayscale_active = false

-- Use this modern Lua block for your grayscale toggle bind
hl.bind(mainMod .. " + SHIFT + G", function()
    if grayscale_active then
        hl.config({ decoration = { screen_shader = "" } })
        grayscale_active = false
    else
        hl.config({ decoration = { screen_shader = "~/.config/hypr/shaders/grayscale.glsl" } })
...
-- Variable to track whether we are customized-fullscreen or not
local browser_fullscreen = false

hl.bind(mainMod .. " + F", function()
    if browser_fullscreen then
        -- If it's already full, reset it back to normal
        hl.dispatch(hl.dsp.window.fullscreen_state({ internal = 0, client = 0 }))
        browser_fullscreen = false
    else
        -- Stretch the window, keep the tabs
...
hl.window_rule({
    name  = "steam",
    match = { initial_class = "(?i)^(steam|bottles)$" },
    workspace = "10",
})
...
---------------------------------------
---- FL STUDIO PASSTHROUGH SUBMAP ----
---------------------------------------

hl.define_submap("flstudio", function()
    -- Press ESCAPE inside the submap to instantly restore your desktop layout
    hl.bind("ESCAPE", hl.dsp.submap("reset"))
    
    -- Keep your multimedia keys active while cooking up beats
    hl.bind("XF86AudioRaiseVolume", hl.dsp.exec_cmd("wpctl set-volume -l 1 @DEFAULT_AUDIO_SINK@ 5%+"), { locked = true, repeating = true })
    hl.bind("XF86AudioLowerVolume", hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%-"),   { locked = true, repeating = true })
    hl.bind("XF86AudioMute",        hl.dsp.exec_cmd("wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle"),  { locked = true })
end)

-- Press ALT + Caps_Lock in your normal desktop environment to enter FL Studio mode
hl.bind(mainMod .. " + Caps_Lock", hl.dsp.submap("flstudio"))
